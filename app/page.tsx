...
export default function Home() {
  ...
  return (
    <div className="shell">
      ...
      <style jsx>{`
        .trade-layout { display: grid; grid-template-columns: 320px 1fr; gap: var(--space-4); height: 100%; overflow: hidden; }
        .col-left { display: flex; flex-direction: column; height: 100%; }
        .col-main { display: grid; grid-template-rows: 1fr 260px; gap: var(--space-4); height: 100%; overflow: hidden; }
        
        .terminal-panel { display: flex; flex-direction: column; overflow: hidden; }
        .terminal-tabs { display: flex; background: var(--bg-primary); border-bottom: 1px solid var(--border); }
        .terminal-tabs button { padding: var(--space-2) var(--space-4); font-size: 9px; font-weight: 800; border: none; border-right: 1px solid var(--border); color: var(--fg-muted); cursor: pointer; background: transparent; transition: var(--transition); }
        .terminal-tabs button.active { background: var(--bg-secondary); color: var(--accent); }
        .terminal-content { flex: 1; overflow-y: auto; background: var(--bg-secondary); }
